...
.setName("panel_channel")
                ...
                .addStringOption((option) =>
                    option
                        ...
                        .setDescription(
                            "تسمية زر إنشاء التكت (الافتراضي: إنشاء تكت)",
                        )
                        .setRequired(false),
                )
                .addChannelOption((option) =>
                ...
                .addIntegerOption((option) =>
                    option
                        ...
                        .setDescription("الحد الأقصى لعدد التكت التي يمكن للمستخدم إنشاؤها (الافتراضي: 1)")
                        .setMinValue(1)
                        .setMaxValue(10)
                        .setRequired(false),
                )
                .addBooleanOption((option) =>
                    option
                        .setName("dm_on_close")
                        .setDescription("إرسال رسالة خاصة إلى المستخدم عند إغلاق تكت (الافتراضي: صحيح)")
                        .setRequired(false),
                ),
        )
        .addSubcommand((subcommand) =>
    ...
    async execute(interaction, config, client) {
        ...
        if (subcommand === "setup") {
            ...
            if (existingConfig?.ticketPanelChannelId) {
                return await replyUserError(interaction, { type: ErrorTypes.UNKNOWN, message: `يحتوي هذا الخادم بالفعل على نظام تذاكر مُعدّ (لوحة التحكم في <#${existingConfig.ticketPanelChannelId}>).\n\nلا يدعم الخادم سوى نظام تذاكر واحد. استخدم \`/ticket dashboard\` لتعديل أو تحديث الإعداد الحالي، أو حدد **حذف النظام** من لوحة التحكم لإزالته والبدء من جديد.` });
            }
...
.setLabel(buttonLabel)
            ...
            try {
                ...
                if (categoryChannel) {
                    ...
                    successMessage += 'سيتم إنشاء التكت الجديدة في مكان جديد"Tickets" فئة.';
                }
                ...
                successMessage += `\n\n**الحد الأقصى للتكت لكل مستخدم:** ${maxTicketsPerUser}\n**رسالة خاصة عند الإغلاق:** ${dmOnClose ? 'مُفعّل' : 'عاجز'}`;
                ...
                const logEmbed = createEmbed({
                    ...
                    .addFields(
                        ...
                        {
                            name: "الحد الأقصى للتكت لكل مستخدم",
