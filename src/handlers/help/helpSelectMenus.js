...
async function createCategoryCommandsMenu(category, client) {
    ...
    const embed = createEmbed({
        title: `${icon} ${categoryName} الأوامر`,
        description: categoryCommands.length > 0
            ? `انقر على أي أمر مذكور أدناه لاستخدامه.`
            : `لم يتم العثور على أي أوامر في **${categoryName}** فئة.`
    });
...
export async function createAllCommandsMenu(page = 1, client) {
    ...
    const embed = createEmbed({
        title: "📋 جميع الأوامر",
        description: `استعرض جميع الأوامر المتاحة في قائمة واحدة. استخدم أزرار الصفحات أدناه للتنقل بين جميع الأوامر..`
    });
